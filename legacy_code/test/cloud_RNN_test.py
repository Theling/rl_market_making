...
from algos.ddpg_rnn_cloud_algo import DSPG
from env.cloud_cartpole_RNN import CartPole
from env.fake_env import fake_env

from policy.RNNpolicy import Policy
from qfunction.RNN_qfunction import QFunction
import numpy as np
...
env = CartPole(1,4,identifier = 'cart_pole', user='zhiyuan', time_step=0.1)
# env = fake_env(1,4)
...
sess = tf.Session()
policy = Policy(sess, state_dim, action_dim, action_bound,time_steps, batch_size,0)
qf = QFunction(sess, state_dim, action_dim, time_steps,0,batch_size, num_actor_vars=len(policy.network_params))
tmp_length = len(policy.network_params) + len(qf.network_params)
target_policy = Policy(sess, state_dim, action_dim, action_bound, time_steps,batch_size,0,num_var_before=tmp_length)
tmp_length += len(target_policy.network_params)
target_qf = QFunction(sess, state_dim, action_dim, time_steps,0,batch_size, num_actor_vars=tmp_length)
