...
class SimpleReplayPool():
    ...
    def add_sample(self, value, terminal, observation, orderbook, action, reward):
    ...
    def _compute_A(self, nextvpred):
        ...
        self._orderbooks = np.array(self._orderbooks, dtype=np.float32)
...
class PPO:
    ...
    def train(self, itr = None):
        ...
        while True:
            ...
            for i in range(self.max_local_step):
                ...
                pool.add_sample(value, terminal, observation[1], observation[0], action, reward)
                observation = next_ob
            ...
            ob, od, ac, atarg, tdlamret = pool._observations, pool._orderbooks, pool._actions, pool._advantage, pool._target_values
            ...
            d = Dataset(dict(ob=ob, od = od,  ac=ac, atarg=atarg, vtarg=tdlamret), shuffle=True)
    ...
    def do_training(self, batch, step):
        ...
        _, total_loss, surrogate, value_loss, entropy_loss = policy_update(obss,ods,  actions, advantages, target_values,
                                                                           step)
