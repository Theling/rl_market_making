...
class OrderBook:
    ...
    def get_Orderbook(self,nRange):
        ret = self.df[['price', 'size', 'type']]
        ret = ret.replace('Ask', 1)
        ret = ret.replace('Bid', -1)
        n_ask = self.n_ask
        n_bid = self.n_bid
        ret = ret.values
        # print(f'ret: {ret}')
        ...
        if n_ask < nRange:
            diff = nRange - n_ask
            for i in range(diff):
                ret = np.vstack((np.array([ret[0][0]+0.01, 0, 1]), ret))
        if n_bid < nRange:
            diff = nRange-n_bid
            for i in range(diff):
                ret = np.vstack((ret, np.array([ret[-1][0]-0.01, 0, -1])))
...
class SHIFT_env:
    def __init__(self,
        ...
        self._cols = ['remained_shares', 'remained_time', 'reward', 'order_type','is_buy', 'premium',
    ...
    def step(self, action):
        ...
        if self.remained_time > 0:
            ...
            reward = exec_share*0 - exec_share * self.commission * 100
            done = True
        ...
        if self.isSave:
            print(f'{self.tmp_obs[1].tolist()}')
            tmp = self.tmp_obs[1].tolist()+[reward, orderType, self.isBuy,
    ...
    def _get_obs(self):
        self.tmp_obs = [self.compute()[0], np.array([self.remained_share, self.remained_time])]
        # print(f'tmp_obs: {self.tmp_obs}')
        return self.tmp_obs
    ...
    def compute(self):
        ...
        feature = []
        if tab.isFull():
            # print(f'len of table: {tab._counter}')
            for ele in tab.getData():
                # print(ele)
                feature.append(ele.get_Orderbook(self.ODBK_range))
