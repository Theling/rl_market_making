...
def plot_premium_bar(data, x='remained_time', y='remained_shares', premium='premium', name='1', max_time=5, max_share=30):
    """
    ...
    ax.set_zlabel('Saving per traded share')

    plt.show()
    fig.savefig('premium_bar_plot_' + name + '.png', dpi=200)
    plt.close(fig)

def plot_premium_ts(data, premium='premium1', remained_s='remained_shares_buy', name='1', max_time=5, max_share=30):
    """
    data: DataFrame, the data used to plot
    name: string, name label of the plot
    max_time: int, the maximum remaining time
    ...
    yy = data.loc[:, premium].values + 0.01
    data_color = data.loc[:, remained_s].values / max_share
    count = 0
    tick_pos = []
    for i in trunc:
...
#-----------------------------------------------------------------------------------------------------------------------------------------
data = pd.read_csv('/mnt/c/Users/liuyu/PycharmProjects/FE800/RL/data/mkt_making/2019-04-12_11-59-38/iteration_info/itr_1.csv')
for i in range(2, 30):
    data1 = pd.read_csv('/mnt/c/Users/liuyu/PycharmProjects/FE800/RL/data/mkt_making/2019-04-12_11-59-38/iteration_info/itr_' + str(i) + '.csv')
    data = pd.concat([data, data1], ignore_index=True)
plot_premium_bar(data, x='remained_time', y='remained_shares_buy', premium='premium1', max_time=5, max_share=6)
plot_premium_bar(data, x='remained_time', y='remained_shares_sell', premium='premium2', max_time=5, max_share=6)


data = pd.read_csv('/mnt/c/Users/liuyu/PycharmProjects/FE800/RL/data/mkt_making/2019-04-12_11-59-38/iteration_info/itr_30.csv')
plot_premium_ts(data, premium='premium1', remained_s='remained_shares_buy', max_time=5, max_share=6)
plot_premium_ts(data, premium='premium2', remained_s='remained_shares_sell', max_time=5, max_share=6)
