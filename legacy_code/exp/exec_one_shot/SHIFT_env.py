...
class SHIFT_env:
    ...
    def step(self, action):
        ...
        if self.remained_share > 0:
            ...
            mktOrder = shift.Order(orderType, self.symbol, self.remained_share)
            self.trader.submitOrder(mktOrder)
            print(f'submited: {mktOrder.symbol}, {mktOrder.type}, {mktOrder.price}, {mktOrder.size}, {mktOrder.id}, {mktOrder.timestamp}')
        if premium > 0:
