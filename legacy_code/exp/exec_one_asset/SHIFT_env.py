...
class SHIFT_env:
    def __init__(self,
        ...
        self._cols = ['BA_spead', 'last_traded_price', 'Smart_price', 'Liquidity_imb', 'market_cost',
              'delta_base_price' , 'remained_shares', 'remained_time', 'reward', 'order_type','is_buy', 'premium',
    ...
    def step(self, action):
        ...
        if self.remained_time > 0:
            reward = exec_share * signBuy * (self.basePrice - obj_price) * 100 + exec_share * self.rebate * 100
        else:
            reward = exec_share * signBuy * (self.basePrice - base_price) * 100 + exec_share * self.commission * 100 # Market order execution
            done = True
...
if __name__=='__main__':
    ...
    env = SHIFT_env(trader, 10, 1, 5, 'AAPL', 0.003)
    # env.close_all()
