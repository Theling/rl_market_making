...
class SHIFT_env:
    ...
    def step(self, action):
        ...
        action = action[0]
        self.currentPos = self._getCurrentPosition()
        self.objPos = self.currentPos + action
        ...
        order = shift.Order(orderType, self.symbol, abs(action))
        self.trader.submitOrder(order)
        print(f'submited: {order.symbol}, {order.type}, {order.price}, base: {order.size}, {order.id}, {order.timestamp}')
        time.sleep(self.timeInterval)
        ...
        rwd = self.reward(self.pv, cv)

        if self.isSave:
            tmp = self.tmp_obs.tolist()+[rwd, orderType, self.isBuy, done]
            # print('-------------', self.tmp_obs)
...
if __name__=='__main__':
    ...
    obj_share = -1
    env.set_objective(share=obj_share)
    # for  i in range(1000):
    #     print(f'time step {i}')
