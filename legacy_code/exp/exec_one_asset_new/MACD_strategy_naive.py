...
dspg = PPO(env=env,
                ...
                session=sess,
            restore_fd='2019-03-13_18-16-02',
...
q = queue.Queue()
rwd = []
port_value = [[0, datetime.datetime.now(), 1000000, 0]]
while trader.isConnected():
    ...
    port_value.append([count, datetime.datetime.now(), portfolioValue(trader), port_shares])
    print(f'Time Step: {port_value[-1][0]}, Time: {port_value[-1][1].time()}, Portfolio Value: {port_value[-1][2]}')
    try:
        rwd.append([count, datetime.datetime.now(), q.get(block=False)])
        print(f'Reward: {rwd[-1][-1]}')
    except queue.Empty:
        rwd.append([count, datetime.datetime.now(), commision])
    ...
    if count > 3600:
        ...
        port_value.append([count, datetime.datetime.now(), final_pv, curr_pos])
        break
...
if isBenchmark:
    port_value.to_csv('Portfolio Value time series.csv')
    rwd.to_csv('Reward time series.csv')
else:
    port_value.to_csv('Portfolio Value time series_Execution.csv')
