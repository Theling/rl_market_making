...
class SHIFT_env:
    def __init__(self,
        ...
        self.act_dict = dict(zip(range(0, self.action_num()), 
                                 np.arange(0.01, 0.01*(1+self.action_num()), 0.01)))
    ...
    def step(self, act_idx):
        ...
        action = self.act_dict[act_idx]
        premium = action
        print(f'action_idx: {act_idx}, premium: {premium}')
    ...
    def _mid_price(df, n_ask):
        return (df.price[n_ask - 1] + df.price[n_ask])/2

    @staticmethod
    def _distance_to_midpoint(df, mid_price):
        return (df.price/mid_price - 1).tolist()# TODO


    @staticmethod
    def _cumulative_value(df):
        return (df.price*df.size).mean()

    @staticmethod
    def _notional_imbalances(df, n_ask, n_bid, act_direction):
        if n_ask > n_bid:
            chi_ask = (df['size'][(n_ask - n_bid):n_ask]*df['price'][(n_ask - n_bid):n_ask]).sum()
            chi_bid = (df['size'][n_ask:]*df['price'][n_ask:]).sum()
        else:
            chi_ask = (df['size'][0:n_ask]*df['price'][0:n_ask]).sum()
            chi_bid = (df['size'][n_ask:(2 * n_ask)]*df['price'][n_ask:(2 * n_ask)]).sum()
        imbal = (chi_ask-chi_bid)/(chi_ask+chi_bid)
