...
class SimpleReplayPool():
    ...
    def _compute_A(self, nextvpred):
        ...
        self._actions = np.array(self._actions, dtype=np.int32)
...
class PPO:
    ...
    def train(self, itr = None):
        ...
        while True:
            ...
            for i in range(self.max_local_step):
                ...
                action, value, act_idx = self.policy.get_a_v(observation)
                ...
                pool.add_sample(value, terminal, observation, act_idx, reward)
                observation = next_ob
                terminal = next_terminal
                # print(terminal)
                if terminal:
                    print(f'{i}: taken step {step}')
                    steps.append(step)
                    step = 0
                    sign *= -1
                    if obj_share < 0:
    ...
    def init_opt(self):
        advantage = keras.Input(dtype= tf.float32, shape = [])
        target_value = keras.Input(dtype= tf.float32, shape = [])
        lrmult = tf.placeholder(name='lrmult', dtype=tf.float32, shape=[])
        ep = self.epsilon * lrmult
        action  = keras.Input(name = 'action', dtype= tf.float32, shape = (self.policy.action_num, ))
        ratio = tf.math.reduce_sum((self.policy.act_dist/self.old_policy.act_dist)*action, axis = 1)
        ...
        value = tf.math.reduce_sum(self.policy.values*action, axis = 1)
        value_loss = tf.reduce_mean(tf.square(value - target_value))
        entropy_loss = -self.policy.act_dist_entropy
        ...
        def policy_update(obs, act, adv, t_v, step):
            cur_lrmult = 1
            return self.sess.run((policy_updt, total_loss, surrogate, value_loss, entropy_loss, self.policy.act_dist)
                                 , feed_dict={self.policy.state: obs,
                                              self.old_policy.state: obs,
                                              action: self.policy.action_encode(act),
                                              advantage: adv,
                                              target_value: t_v,
                                              lrmult: cur_lrmult})
    ...
    def do_training(self, batch, step):
        ...
        _, total_loss, surrogate, value_loss, entropy_loss, act_dist = policy_update(obss, actions, advantages, target_values,
                                                                           step)
        ...
        logger.log('action dist: %s' % act_dist)
