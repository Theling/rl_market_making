...
class SimpleReplayPool():
    ...
    def _compute_A(self, nextvpred):
        ...
        print(f'tgt_v: {self._target_values.shape}')
        self._target_values = self._advantage + self._target_values
        print(f'adv: {self._advantage.shape}')
        print(f'tgt_v: {self._target_values.shape}')
        print(f'act: {self._actions.shape}')
        print(f'obs: {self._observations.shape}')
...
class PPO:
    ...
    def init_opt(self):
        ...
        target_value = tf.placeholder(tf.float32, [None], name = 'tgt_v')
        lrmult = tf.placeholder(name='lrmult', dtype=tf.float32, shape=[])
        ep = self.epsilon * lrmult
        action = tf.placeholder(tf.float32, [None] + [self.policy.action_dim], name = 'act')
    ...
    def do_training(self, batch, step):
        ...
        # obss = self.normalize(np.array(obss), 0)
