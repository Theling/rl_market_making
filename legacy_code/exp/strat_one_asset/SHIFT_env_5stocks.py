...
class SHIFT_env:
    def __init__(self,
        ...
        self.dataThread = Thread(target=self.thread_map)
        # features = ['symbol', 'orderPrice', 'orderSize', 'OrderTime']
        self.table = CirList(nTimeStep)
        self._cols = ['BA_spead', 'last_traded_price', 'Smart_price', 'Liquidity_imb', 'market_cost',]*5+\
    ...
    def _link(self, symbol):
        ...
        last_price = self.trader.getLastPrice(symbol)

        Ask_ls = self.trader.getOrderBook(symbol, shift.OrderBookType.GLOBAL_ASK, self.ODBK_range)
        # assert Ask_ls, f'getOrderBook: return empty list: {self.symbol}-ASK-{self.ODBK_range}'

        Bid_ls = self.trader.getOrderBook(symbol, shift.OrderBookType.GLOBAL_BID, self.ODBK_range)
    ...
    def thread_map(self, pool=5):
        res = {}
        order = []
        env = SHIFT_env(trader,scanner_wait_time = 1,
                    decision_gap = 10,
                    nTimeStep = 10,
                    ODBK_range = 10,
                    symbol = ['IBM','CAT','DIS','GS','TRV'],
                    executioner = exec)
        while self.trader.isConnected() and self.thread_alive:
            if pool is None:
                def target(arg, num):
                    try:
                        res[num] = env._link(symbol=arg)
                    except:
                        res[num] = sys.exc_info()

                threads = [Thread(target=target, args=[arg, i]) for i, arg in enumerate(self.symbol)]
            else:
                ...
                queue = Queue()
                for i, arg in enumerate(self.symbol):
                    queue.put((i, arg))
            ...
            print(f'Order Book Data:{order}')

            self.mutex.acquire()

            self.table.insertData(order)
    ...
    def reset(self):
        for i in range(len(self.symbol)):
            print(f'Holding shares: {self.trader.getPortfolioItem(self.symbol[i]).getShares()}')
        ...
        if tab.isFull():
            for ele in tab.getData():
                ...
                for i in range(5):
                    ...
                    bas = self._ba_spread(ele[i].df, n_ask)
                    p = self._price(ele[i].df)
                    sp = self._smart_price(ele[i].df, n_ask)
                    li = self._liquid_imbal(ele[i].df, n_ask, n_bid, act_direction)
                    if act_direction:
                        mc, _ = self._market_cost(ele[i].df,
                                                  n_ask,
                                                  n_bid,
                                                  act_direction,
                                                  delta,
                                                  self.commission)
                        feature_ls+=[bas, p, sp, li, mc]
                        #print(feature_ls)
                    else:
                        feature_ls+=[bas, p, sp, li, np.nan]
            feature.append(feature_ls)
    ...
    def _getCurrentPosition(self):
        ...
        for i in self.symbol:
            x+=int(self.trader.getPortfolioItem(i).getShares() / 100)
        return x
...
if __name__=='__main__':
    ...
    df = pd.DataFrame(index=['row'],
                      columns=['Time']+['BA_spread', 'Price', 'Smart_price', 'Liquid_imb', 'MKT_cost']*5)
    df_save_name = 'Daily_data.csv'
    ...
    for i in range(360*7):
        ...
        for j in range(len(obs)):
            df.loc['row'] =[datetime.datetime.now(),obs[j,0], obs[j,1]*1000, obs[j,2]*1000, obs[j,3], obs[j,4],obs[j,5], obs[j,6]*1000, obs[j,7]*1000, obs[j,8], obs[j,9],obs[j,10], obs[j,11]*1000, obs[j,12]*1000, obs[j,13], obs[j,14],
                             obs[j,15], obs[j,16]*1000, obs[j,17]*1000, obs[j,18], obs[j,19],obs[j,20], obs[j,21]*1000, obs[j,22]*1000, obs[j,23], obs[j,24]]
