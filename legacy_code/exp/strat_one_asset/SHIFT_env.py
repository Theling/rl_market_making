...
class SHIFT_env:
    def __init__(self,
        ...
        self._cols = ['BA_spead', 'last_traded_price', 'Smart_price', 'Liquidity_imb', 'market_cost', 'log_r(obs)',
                'reward(log_r)','port_v','last_savings', 'delta_shares',  'done', 'Time']
    ...
    def step(self, action):
        ...
        if self.isSave:
            tmp = next_obs[-1].tolist()+[rwd, cv, savings, delta_shares, done, datetime.datetime.now()]
            # print('-------------', self.tmp_obs)
    ...
    def compute(self, delta):
        ...
        tmp = np.log(feature[1:,1]/feature[0:-1,1])
        tmp = tmp.reshape(-1,1)
        # print(feature.shape, tmp.shape)
        feature = np.hstack((feature[1:,:], tmp))
        return feature
...
class Oneshot_exec:
    ...
    def execute(self, share, _):
        originPos = self._getCurrentPosition()
        if share > 0:
            orderType = shift.Order.LIMIT_BUY
        else:
            orderType = shift.Order.LIMIT_SELL
    ...
    def _cancelAllOrder(self, order):
        ...
        tmp_con = 0
        self.trader.submitCancellation(order)
        print('Canceling order:', end='')
...
if __name__=='__main__':
    ...
    obs = env.reset()
    pv = pd.DataFrame([[datetime.datetime.now(),1000000,0]], index=['row'], columns=['Time','PortfolioValue','CurrentShares'])
    ...
    for i in range(360*7):
        ...
        for j in range(len(obs)):
            df.loc['row'] = [datetime.datetime.now(),obs[j,0], obs[j,1]*1000, obs[j,2]*1000, obs[j,3], obs[j,4]]
        ...
        print(f'holding shares: {trader.getPortfolioItem("DWDP").getShares()}')
        print(obs)
        print(reward)
        total_reward += reward
        print(done)
    ...
    print(trader.getPortfolioItem('DWDP').getShares())
