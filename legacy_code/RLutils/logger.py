...
def init(restore = False, dir_name = None, session = None, itr = 'latest'):
    ...
    if restore:
        ...
        if itr == 'latest':
            ...
            itr_ls = [int(latest_file[(latest_file.find('_') + 1):latest_file.find('.')]) \
                for latest_file in file_ls if '.' in latest_file ]
            if _snapshot_mode == 'last':
                latest_itr = None
            else:
                latest_itr = max(itr_ls)
...
def _restore(itr, session):
    if _snapshot_mode == 'all' or _snapshot_mode == "gap":
        ...
        cprint(f'Model is restored from {file_name}, {file_name2}')
