...
class PPO:
    ...
    def train(self):
        ...
        epoch = 0
        if self.restore_fd:
            info = logger.init(restore=True, dir_name=self.restore_fd, session=self.sess, itr=600)
            epoch = info['epoch']
        else:
            logger.init()
    ...
    def init_opt(self):
        ...
        policy_optimizer = tf.contrib.opt.ScipyOptimizerInterface(total_loss, var_list=var_list, options={'maxiter': 2})
        init = tf.global_variables_initializer()
        self.sess.run(init)

        def policy_update(obs, act, adv, t_v, step):
            cur_lrmult = 1
            policy_optimizer.minimize(self.sess, feed_dict={self.policy.state: obs,
                                              self.old_policy.state: obs,
                                              action: act,
                                              advantage: adv,
                                              target_value: t_v,
                                              lrmult: cur_lrmult})
            return self.sess.run((clipped_ratio, total_loss, surrogate, value_loss, entropy_loss)
